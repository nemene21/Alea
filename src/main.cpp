...
void draw_foreground(float delta) {
    SceneManager::scene_on->draw(delta);
    SceneManager::scene_on->draw_entities(delta);
    SceneManager::scene_on->late_draw(delta);
}
...
int main() {
    ...
    // Init
    InitWindow(res.x*12, res.y*12, "Aleae");
    ...
    AudioManager::play_track("music.wav", 1);

    SceneManager::setup_scene(new GameScene(true));
